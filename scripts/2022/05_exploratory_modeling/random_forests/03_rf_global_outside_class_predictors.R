# --------------------------------------
# Random Forest Model: Global Outside-Class Predictors Only
# --------------------------------------
# This RF Script:
# - - Runs random forest models using only non-input, non-COVID, Included predictors
#   labeled "Out of School" or "In and Out" in the broad_learning_context column
# - Excludes COVID-related variables, PVs, weights, WLEs, and identifier columns.
# - Includes both questionnaire and derived/composite variables.
# - Computes variable importance using mean increase in node purity.
# - Aggregates importance across PVs and outcomes to identify top predictors.
# - Generates Partial Dependence Plots (PDPs) for the global top 5 predictors
#   based on normalized importance across PV1–10 × all three outcomes.
# - Saves raw importance tables, aggregated tables, visual plots, and PDPs.
# - Fully updates the master model tracker for reproducibility and audit trail.
# --------------------------------------
...
model_id <- "outside_class"

output_folder_base <- file.path("output/private/exploratory_data_analysis/2022/random_forests/global", model_id)
...
run_rf_for_pv <- function(pv_name, df, predictors, weight_var) {
  df_sampled <- df %>%
    ...
    slice_sample(n = 5000, weight_by = !!sym(weight_var))
...
for (outcome in outcomes) {
  ...
  tracker_row <- tibble(
    ...
    predictors_used = "All valid outside-class questionnaire and derived variables (excluding COVID variables)",
  ...
  function(subject) {
    map_dfr(1:10, function(pv_num) {
      ...
      df %>%
        ...
        slice_sample(n = 5000, weight_by = W_FSTUWT) %>%
        rename(score = !!target) %>%
...
# --------------------------------------------------
# Compare Top 20 Predictors Across Global RF Models
# --------------------------------------------------
compare_top20_paths <- list(
  all = "output/private/exploratory_data_analysis/2022/random_forests/global/all_predictors/global_all_predictors_rf_top20_predictors_by_importance.csv",
  in_class = "output/private/exploratory_data_analysis/2022/random_forests/global/in_class/global_in_class_rf_top20_predictors_by_importance.csv",
  outside_class = "output/private/exploratory_data_analysis/2022/random_forests/global/outside_class/global_outside_class_rf_top20_predictors_by_importance.csv"
)

# Load all three top20 tables
top20_all <- read_csv(compare_top20_paths$all, show_col_types = FALSE) %>% 
  mutate(rank_all = row_number()) %>% 
  rename(norm_imp_all = mean_norm_importance) %>%
  mutate(norm_imp_all = round(norm_imp_all, 3))

top20_in <- read_csv(compare_top20_paths$in_class, show_col_types = FALSE) %>% 
  mutate(rank_in = row_number()) %>% 
  rename(norm_imp_in = mean_norm_importance) %>%
  mutate(norm_imp_in = round(norm_imp_in, 3))

top20_out <- read_csv(compare_top20_paths$outside_class, show_col_types = FALSE) %>% 
  mutate(rank_out = row_number()) %>% 
  rename(norm_imp_out = mean_norm_importance) %>%
  mutate(norm_imp_out = round(norm_imp_out, 3))

# Merge all tables
top_combined <- full_join(top20_all, top20_in, by = "variable") %>% 
  full_join(top20_out, by = "variable") %>% 
  mutate(
    delta_rank_in = rank_all - rank_in,
    delta_rank_out = rank_all - rank_out
  ) %>% 
  arrange(rank_all, rank_in, rank_out)

# Final table: Cleaned and ordered
final_top20_comparison <- top_combined %>% 
  select(
    variable,
    rank_all, norm_imp_all,
    rank_in, norm_imp_in, delta_rank_in,
    rank_out, norm_imp_out, delta_rank_out
  )

# Save to CSV
comparison_file <- "output/private/exploratory_data_analysis/2022/random_forests/global/global_rf_top20_comparison_table.csv"
write_csv(final_top20_comparison, comparison_file)

# Preview to console
cat("\n📋 Top 20 Comparison Table Preview:\n")
print(head(final_top20_comparison, 10))

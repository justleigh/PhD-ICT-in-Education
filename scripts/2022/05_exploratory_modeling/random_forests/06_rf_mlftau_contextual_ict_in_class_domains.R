# --------------------------------------
# MLFTAU Contextual ICT In-Class Domains (All Predictors)
# --------------------------------------
# This Random Forest (RF) script:
# - Models student achievement across MATH, READ, and SCIE using:
#     • ICT-related variables assigned to the MLFTAU ICT In-School framework
#     • General contextual variables mapped to MLFTAU domains
# - Restricts analysis to predictors labeled as:
#     • "In School" or "In and Out" in the broad_learning_context column
#     • "Included" in the status column
#     • Non-input or derived variables (input_variable == "No" | derived_variable == "Yes")
# - Excludes:
#     • COVID-related variables (pisa_construct == "Global Crises")
#     • Dependent Variables (DVs), weights as predictors, PVs, and identifiers
# - For each MLFTAU domain, computes:
#     • Raw and average variable importance scores across PV1–PV10
#     • Normalized importance tables and barplots (Top 20)
#     • Combined normalized plots (across all outcomes)
#     • Partial Dependence Plots (PDPs) for the Top 5 predictors
...
output_folder_base <- "output/private/exploratory_data_analysis/2022/random_forests/mlftau_contextual_ict_in_class/domains"
...
for (i in seq_along(domains_capitalized)) {
  ...
  for (outcome in outcomes) {
    ...
    tracker_row <- tibble(
      ...
      plot_file = plot_file,
      framework = framework,  # Already dynamic

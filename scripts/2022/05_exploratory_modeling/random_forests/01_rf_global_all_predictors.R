# --------------------------------------
# Random Forest Model: Global All Predictors
# --------------------------------------
# This RF Script:
# - Runs random forest models using all non-input, non-COVID, Included predictors
#   for all 10 plausible values of mathematics, reading, and science.
...
for (outcome in outcomes) {
  ...
  tracker_row <- tibble(
    ...
    predictors_used = "All valid questionnaire and derived variables (excluding COVID variables)",
    subset_rule = "All Included non-input predictors",
    domain_source_column = "None",   # ← 🔁 e.g., "MLFTAU Construct" or "PISA Domain"
...
importance_files <- list.files(
  file.path(output_folder_base, "variable_importance_tables"),
  pattern = paste0("_", model_id, "\\.csv$"),     # only get in_class files
  full.names = TRUE
)
...
for (var in global_top5) {
  ...
  ggsave(
    filename = file.path(output_folder_base, "plots/pdp_plots", paste0("global_pdp_", var, ".png")),
    plot = pdp_plot,

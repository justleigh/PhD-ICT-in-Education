# --------------------------------------
# Random Forest Model: PISA Contextual ICT Outside-School Domains
# --------------------------------------
# This RF Script:
# - Uses all non-input predictors related to outside-school learning contexts, including:
#     • ICT-related variables from the PISA ICT Outside-Class framework
#     • General contextual variables from the main PISA Questionnaire Framework
# - Includes only questionnaire and derived variables labeled for 
#   "Out of School" or "In and Out" learning contexts
# - Excludes COVID-related variables, DVs, Plausible Values (PVs), weights as predictors, and identifier columns
# - Computes average variable importance across all PVs (PV1–PV10) for each outcome (MATH, READ, SCIE)
# - Outputs:
#     • Raw and averaged importance tables per outcome
#     • Top 20 variable importance barplots per outcome
#     • Combined top 20 normalized importance plots per domain
#     • Partial Dependence Plots (PDPs) for top 5 domain-level predictors (averaged across PVs and outcomes)
...
output_folder_base <- "output/private/exploratory_data_analysis/2022/random_forests/pisa_contextual_ict_outside/domains"
...
select_predictors_for_domain <- function(domain_label) {
  mapping %>%
    filter(
      str_detect(
        !!sym(domain_source_column),
        regex(paste0("(^|;)\\s*", domain_label, "\\s*(;|$)"))
      ),

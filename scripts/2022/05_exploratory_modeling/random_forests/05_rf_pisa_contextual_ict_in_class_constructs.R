# --------------------------------------
# Random Forest Model: PISA Contextual ICT In-Class Constructs
# --------------------------------------
# This RF Script:
# - Identifies all non-input predictors related to in-school learning contexts using two frameworks:
#     • ICT-related variables defined by the PISA ICT In-Class Framework
#     • General contextual variables from the PISA Questionnaire Framework that are not labeled ICT-related
# - Contextual (non-ICT) variables are included only if:
#     • Their broad_learning_context is "In School" or "In and Out"
#     • Their construct and domain are taken from pisa_construct and pisa_domain
#     • Their construct is prefixed with "context_" for clear distinction
# - ICT-related variables are mapped using:
#     • pisa_ict_school_construct and pisa_ict_school_domain columns
# - Excludes COVID-related variables, DVs, plausible values (PVs), and identifiers
# - Excludes weights as predictors (but uses W_FSTUWT for weighted resampling)
# - Computes Random Forest models using PV1–PV10 for each outcome: MATH, READ, and SCIE
# - Organizes outputs by construct and its associated domain(s), with directory structure:
#     • /constructs/{domain}/{construct}/
#     • Raw and averaged importance tables per outcome
#     • Top 20 variable importance barplots per outcome
#     • Combined top 20 normalized importance plots per construct
# - Updates the master model tracker with metadata about each model run
# --------------------------------------
...
output_folder_base <- "output/private/exploratory_data_analysis/2022/random_forests/pisa_contextual_ict_in_class/constructs"
...
select_predictors_for_construct <- function(construct_label, domain_label) {
  ...
  if (str_starts(construct_label, "context_")) {
    ...
    filter_column <- "pisa_ict_school_construct"
    domain_column <- "pisa_ict_school_domain"
  }
  ...
  # Final variable selection (NOW correctly scoped)
...
ict_constructs <- mapping_filtered %>%
  filter(
    !pisa_ict_school_construct %in% c("", "Not Applicable")
  ) %>%
  pull(pisa_ict_school_construct) %>%
  str_split(";") %>%
  unlist() %>%
  str_trim() %>%
...
contextual_constructs <- mapping_filtered %>%
  filter(pisa_ict_school_construct %in% c("", "Not Applicable")) %>%
  pull(pisa_construct) %>%
